...
proc get_xml_content {file_conntent} {
  ...
  foreach line $file_content_list {
    ...
    set label [lindex [split $raw_tokens ">"] 0]
    set token [lindex [split $raw_tokens ">"] 1]
    if { $label != "tokens" && $token != "" } {
      puts "$token $label" 
      lappend tokens_list [list $label $token]
    }
  }
  return $tokens_list
}

proc parsed_tokens {tokens_list} {
  set parsed_tokens_tree [list]
}
